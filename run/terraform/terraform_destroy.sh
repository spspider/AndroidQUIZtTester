...
if [ -z "$id" ]; then
    id="$(jq '[.resources[].instances[].attributes.id]  | .[]' terraform.tfstate.backup)"
    name="$(jq '[.resources[].instances[].attributes.tags.Name]  | .[]' terraform.tfstate.backup)"
fi
